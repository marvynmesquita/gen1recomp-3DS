...
function love.load(args)
  ...
  -- Native 3DS: custom two-screen launcher with runtime ROM import (top =
  -- Gen1Recomp logo in 3D + Gen-1 themed background; bottom = JOGO / MODS /
  -- INICIAR + ROM list + import progress bar).  Detected by the _listdir C
  -- bridge that only the 3DS build registers.  Desktop keeps the bundled
  -- launcher below.
  if type(_listdir) == "function" then
    require("src.launcher3ds").boot(bootGame)
    return
  end
