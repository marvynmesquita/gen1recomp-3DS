...
-- optional per-phase draw profiler (src/core/DrawProf.lua); pcall-guarded
-- so a missing module can NEVER break the game (love.draw stays live)
local okDrawProf, DrawProf = pcall(require, "src.core.DrawProf")
if not okDrawProf then DrawProf = nil end
...
function OverworldState:setMap(mapId, x, y, facing, opts)
  ...
  -- settled below, once the map record is in hand.  Only invalidate the current
  -- map and its connected neighbors: invalidating the whole resident cache on a
  -- dark-map toggle rebuilds every atlas in memory and produces the visible fade
  -- + spike seen when crossing route/dark-map boundaries.
  if PaletteFX.setDarkWorld(isDarkMap(mapId) and not Game.save.flashLit)
    ...
    for _, nb in ipairs(self.neighbors or {}) do
      MapLoader.invalidate(nb.map.id)
    end
  end
...
function OverworldState:sgbPalettes()
  local PaletteFX = require("src.render.PaletteFX")
  -- 3DS has no shader support, so no SGB zone is ever consumable here: skip
  -- the per-frame palette-name/zone-table work (blitCanvas fast-paths on the
  -- shader being nil and ignores zone lists entirely).  Desktop/mobile keep
  -- the full path -- their shader() is truthy.
  if not PaletteFX.shader() then return nil end
...
function OverworldState:sgbWorldZones()
  local PaletteFX = require("src.render.PaletteFX")
  -- 3DS: same shader gate as sgbPalettes -- zones are only consumed by the
  -- shade-remap shader, which does not exist there, so the map/neighbour
  -- palette lookups and zone tables are dead work every frame.
  if not PaletteFX.shader() then return nil end
...
function OverworldState:setDark(on)
  ...
  if PaletteFX.setDarkWorld(on) and PaletteFX.usesGbcPack() and self.map then
    MapLoader.invalidate(self.map.id)
    for _, nb in ipairs(self.neighbors or {}) do
      MapLoader.invalidate(nb.map.id)
    end
...
-- Stereoscopic 3D parallax per render layer, in game pixels at FULL 3D
-- slider.  The C++ draw adds slider * love.graphics._parallax * eyeSign to
-- each draw's X, so the two eye views diverge and the 3DS parallax barrier
-- fuses them into depth.  MIN tilt = slider 0 (no depth); MAX tilt = slider
-- 1.0, where the offset reaches these values.  Tiles recess behind the
-- screen, sprites pop in front of it, FX ride just behind the sprites, and
-- the UI / borders stay pinned to the screen plane (0).  Tune to taste --
-- bigger = deeper separation (the C++ loop renders mono when the slider is 0).
local PARALLAX_TILES   = -0.3
local PARALLAX_SPRITES =  0.55
local PARALLAX_FX      =  0.4
local PARALLAX_UI      =  0.0
...
function OverworldState:draw()
  if DrawProf then DrawProf.begin("ow_begin") end
  Game.renderer:beginWorldPass()
  if DrawProf then DrawProf.finish("ow_begin") end
  if DrawProf then DrawProf.begin("ow_world") end
  self:drawWorld()
  if DrawProf then DrawProf.finish("ow_world") end
  if DrawProf then DrawProf.begin("ow_end") end
  love.graphics._parallax = PARALLAX_UI
  Game.renderer:endWorldPass()
  if DrawProf then DrawProf.finish("ow_end") end
  if DrawProf then DrawProf.begin("ow_ui") end
  self:drawUI()
  if DrawProf then DrawProf.finish("ow_ui") end
  -- In stereoscopic 3D the C++ loop calls love.draw twice (once per eye);
  -- count profiler frames only for the left/mono pass so the flush cadence
  -- and per-frame peaks stay per rendered frame.
  if DrawProf and not (love.graphics and love.graphics._eye == 1) then
    DrawProf.frame()
  end
end
...
function OverworldState:drawWorld()
  if DrawProf then DrawProf.begin("ow_ovh") end
  ...
  if not pipelineId then
    if DrawProf then DrawProf.begin("ow_map") end
    love.graphics._parallax = PARALLAX_TILES
    ...
    if DrawProf then DrawProf.finish("ow_map") end
  end
  ...
  elseif not tilt then
    ...
    if DrawProf then DrawProf.begin("ow_spr") end
    love.graphics._parallax = PARALLAX_SPRITES
    for _, g in ipairs(self.ghosts) do
    ...
    if DrawProf then DrawProf.finish("ow_spr") end
    if DrawProf then DrawProf.begin("ow_fx") end
    love.graphics._parallax = PARALLAX_FX
    fxHeal()
    fxDust()
    fxCutTree()
    fxEmote()
    fxBird()
    fxRod()
    if DrawProf then DrawProf.finish("ow_fx") end
  else
  ...
  if DrawProf then DrawProf.finish("ow_ovh") end
end
