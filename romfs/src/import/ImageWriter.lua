...
-- On the 3DS love.image.newImageData is deliberately `false` (the engine
-- gates its slow palette paths on it), so the runtime ROM importer gets a
-- real ImageData factory under love.image._newImageData instead.  Desktop
-- keeps the standard love.image.newImageData.
local function newImageData(w, h)
  if love.image._newImageData then
    return love.image._newImageData(w, h)
  end
...
function ImageWriter.decode2bpp(raw, width, height, transparent)
  ...
  local image = newImageData(width, height)
