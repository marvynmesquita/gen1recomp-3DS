...
-- Non-throttled import trace (native 3DS build only).  print -> sys_log
-- drops lines closer than 250ms apart, but an import stage can finish in
-- less, so the last log.txt line does not always reveal the exact
-- sub-operation that corrupted memory.  This appends every event to its own
-- file with no throttling: on the next hardware crash the LAST line is the
-- exact step that died.
local traceHandle = nil
local function trace(message)
  if type(_listdir) ~= "function" then return end -- native 3DS build only
  if traceHandle == nil then
    local ok, fh = pcall(io.open,
      "sdmc:/3ds/gen1recomp3ds/import_trace.txt", "a")
    traceHandle = ok and fh or false
  end
  if traceHandle then
    pcall(function()
      traceHandle:write(message .. "\n")
      traceHandle:flush()
    end)
  end
end
...
function RomExtractor:beginStage(name)
  ...
  -- One log line per stage.  The 3DS hard-crashes (native abort) on the
  -- memory corruption with no Lua error to point at, so this is what pins a
  -- crash to the exact stage from log.txt between "[import] start" and
  -- "[import] done"; the heap reading flags runaway allocations too.
  print(string.format("[import] stage %d/%d: %s (heap %.1f MB)",
                      self.stage, STAGE_COUNT, name,
                      collectgarbage("count") / 1024))
  trace(string.format("stage %d/%d: %s (heap %.1f MB)",
                      self.stage, STAGE_COUNT, name,
                      collectgarbage("count") / 1024))
...
function RomExtractor:write2bpp(raw, width, height, relative, transparent)
  trace("  decode+save " .. relative .. " (" .. width .. "x" .. height .. ")")
...
function RomExtractor:run()
  -- Every stage serialises its data to disk (self:write / self:save) before it
  -- returns, and neither caller (launcher3ds, RomImporter) reads the returned
  -- results, so keep only the CURRENT stage's output alive instead of
  -- accumulating all of them.  On the 3DS that growth (constants..audio, ~14
  -- MB by the trainer stage) is exactly what exhausts the heap around the
  -- sprite/trainer stages (12-13) and white-screens the console.  Free each
  -- result the moment its stage is done and force a full GC at the boundary.
  local results = {}
  ...
  stage("constants", function() return self:extractConstants() end)
  stage("tilesets", function() return self:extractTilesets() end)
  stage("maps", function() return self:extractMaps() end)
  stage("font", function() return self:extractFont() end)
  stage("sprites", function() return self:extractSprites() end)
  stage("moves", function() return self:extractMoves() end)
  stage("battle_anims", function() return self:extractBattleAnimations() end)
  stage("items", function() return self:extractItems() end)
  stage("type_chart", function() return self:extractTypeChart() end)
  stage("palettes", function() return self:extractPalettes() end)
  stage("icons", function() return self:extractIcons() end)
  stage("pokemon", function() return self:extractPokemon() end)
  stage("trainers", function() return self:extractTrainers() end)
  stage("encounters", function() return self:extractEncounters() end)
  stage("text", function() return self:extractText() end)
  stage("field", function() return self:extractField() end)
  stage("audio", function() return self:extractAudio() end)
  if self.progress then
