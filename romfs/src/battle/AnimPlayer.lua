...
function AnimPlayer:drawSprites(sprites, colorFn)
  ...
  local slices = shader and g.getScissor and g.intersectScissor
                 and g.setScissor
  -- 3DS / headless flat path: no shade shader, so no per-sprite color
  -- sampling, shader sends or scissor slicing -- just the plain draws.
  -- This is the hot path during move animations, which can carry many OAM
  -- sprites; skipping the per-sprite closure and cell math here is cheap
  -- and exactly replicates the blit below.
  if not shader then
    for i = 1, #sprites do
      local s = sprites[i]
      ...
      if s.x > 0 and s.x < 168 and s.y > 0 and s.y < 160 then
        ...
        if quad then
          g.draw(img, quad,
                 s.x - 8 + (s.xf and 8 or 0),
                 s.y - 16 + (s.yf and 8 or 0),
                 0,
                 s.xf and -1 or 1,
                 s.yf and -1 or 1)
        end
