...
-- Stereoscopic 3D: depth is per-layer.  Each scene publishes its own
-- _parallax (game px, scaled by the 3D slider in the C++ per-eye render:
-- X += slider * _parallax * eyeSign) as it draws -- overworld's tiles/
-- sprites/FX/UI, the title's paper/logo/mon/Red, the battle's paper/pics/
-- HUD/anim/text.  Relative depth BETWEEN planes is what reads as 3D; a
-- uniform base made the whole frame shift as one rigid plane, which read
-- as "the screen shrank" instead of depth, so beginFrame pins the screen
-- plane (0) and scenes opt in.
...
function Renderer:beginFrame(transparent)
  ...
  -- 3D baseline: the screen plane every frame.  Scenes publish their own
  -- per-layer _parallax while drawing (see the note by MAX_UI_HEIGHT).
...
function Renderer:endFrame(zones, worldZones)
  ...
  -- remap shader in blitCanvas, so skip building/merging them here
  -- (OverworldState.sgbPalettes/sgbWorldZones return nil for the same
  -- reason).  Desktop and mobile have the shader, so their zone handling
  -- below is untouched.
  ...
  local clearR, clearG, clearB = 1, 1, 1
