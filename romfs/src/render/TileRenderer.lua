...
local WINDOW_MARGIN = 1 -- tiles of slack kept around the view between refills.
-- On the 3DS every batch entry is one C2D_DrawImageAt per frame, so the
-- standing static batch is THE dominant overworld draw cost (a margin of
-- 8 tiles was 1224 entries/frame; 4 was 728; the 20x18 view alone is 360).
-- The margin only delays the next refill -- it does not change the fill
-- cadence, and a refill itself is a cheap batch rebuild.  The +1 in tx1/ty1
-- already covers the partially-visible edge tile during sub-tile scroll, so
-- 1 tile of slack (~440 entries) covers the view with no edge gap and no
-- perceptible refill stutter (a refill fires every ~16px of scroll while
-- walking).  Anything larger is pure per-frame GPU cost on the 3DS.
...
function TileRenderer:ensureWindow(camX, camY, vw, vh)
  ...
  if not self.winBatch then
    self.winBatch = love.graphics.newSpriteBatch(self.image, 2048, "dynamic")
  end
  self.winBatch:clear()
  local anims = self.anims
  for _, anim in ipairs(anims) do
    if not anim.batch then
      anim.batch = love.graphics.newSpriteBatch(anim.textures[1], 512, "dynamic")
    end
    anim.batch:clear()
  end
  ...
  for ty = ty0, ty1 - 1 do
    ...
    for tx = tx0, tx1 - 1 do
      ...
      if block then
        ...
        -- window batch is one redundant GPU draw per tile on the 3DS.  A
        -- GATED anim (spinner) only overdraws while its gate is open, so it
        -- must stay in the static batch to show its static tile when shut.
        if anim and not anim.gate then
          -- skip the static add; the anim batch covers this tile always
        else
