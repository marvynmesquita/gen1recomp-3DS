...
local cache = {}
-- deferred image creation queue: this spreads expensive image loads across many
-- frames to avoid a single transition or map load freezing the render thread.
-- The queue is intentionally main-thread only: love.graphics.newImage must stay
-- off any worker state, and this is the safe next step before broader worker
-- fan-out is added.
local pending = {}
local queue = {}
local active = {}
Assets.batch = 8
Assets.maxPending = 64
...
-- Queue a path for deferred load.  This is intentionally conservative: a path is
-- only staged once, and the queue drains a tiny batch per frame so a map
-- transition does not stall the whole update on a burst of new image loads.
function Assets.queue(path)
  if type(path) ~= "string" then return nil end
  local resolved = Assets.resolve(path)
  if cache[resolved] or active[resolved] or pending[resolved] then return cache[resolved] end
  if not exists(resolved) then return nil end
  pending[resolved] = true
  table.insert(queue, resolved)
  if #queue > Assets.maxPending then
...
-- Convenience wrapper used for a whole batch of one-shot asset loads.
function Assets.preload(paths)
...
function Assets.update()
  ...
  local processed = 0
  while #queue > 0 and processed < Assets.batch do
    local path = table.remove(queue, 1)
    if path then
      pending[path] = nil
      if not cache[path] and not active[path] then
        active[path] = true
        local ok, image = pcall(love.graphics.newImage, path)
        if ok and image then cache[path] = image end
        active[path] = nil
      end
    end
