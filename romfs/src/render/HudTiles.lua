...
-- When a DrawSink is open (battle HUD cache), tile draws are recorded into
-- per-texture SpriteBatches instead of hitting the screen one by one.
local DrawSink = require("src.render.DrawSink")

local function put(t, x, y, tint)
  if not t then return end
  if DrawSink.addToSink(t.img, t.quad, x, y) then return end
  -- This renderer applies no per-vertex tint to images, so an untinted tile
  -- needs no color dance at all (its pixels are already the right color):
  -- skip the getColor/save/setColor/restore and just draw.  Tinted tiles
  -- (the HP-bar fill in the flat path) keep the save/restore.
  if tint then
