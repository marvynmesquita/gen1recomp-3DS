...
function SpriteRenderer:draw(px, py, camX, camY, facing, walkPhase, stepFlip,
  ...
  local redraw = false
  -- True-color sheets bypass every palette bake; the screen-space
  -- exemption is recorded below once the final frame/height is known.
  ...
  elseif PaletteFX.usesGbcPack() then
    ...
    -- terrain, this bakes the sprite) and the world pass runs unshaded, so
    -- this draws like any normal sprite -- opaque character pixels over a
  ...
  elseif PaletteFX.usesSpriteObp() and PaletteFX.spriteRedrawPassActive() then
    ...
    -- palette, baked and replayed over the zone pass (#155/#301).
    image = getObpImage(self.def.image, PaletteFX.ogObj())
    redraw = true
  else
    -- SGB and the mono/inverted modes: the sprite stays in DMG shades and
    -- the whole-canvas zone shader colors it with the map's palette -- the
    -- only thing the Super Game Boy can do to an OBJ (#301).
    -- dmgObj() is mode-invariant (the one DMG OBP ramp), shifting only with
    -- the cave-dark state, so the resolved bake is cached per sprite keyed
    -- on that -- avoids the per-frame path..group concat + palette calls.
    local dk = PaletteFX.darkKey()
    if self._dmgImageKey ~= dk then
      self._dmgImageKey = dk
      self._dmgImage = getObpImage(self.def.image, PaletteFX.dmgObj())
    end
    image = self._dmgImage
  end
