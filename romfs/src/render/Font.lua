...
function Font.drawCode(code, x, y)
  ...
  if quad then
    -- When a DrawSink is open (battle HUD cache), record the glyph instead
    -- of issuing a separate Lua->C++ draw.
    if not DrawSink.addToSink(page.image, quad, x, y) then
...
-- the border glyphs of a box, drawn tile by tile (drawBox / the batched
-- variant's fallback).  Split out of drawBox so drawBoxBatched can share it.
local function drawBorder(tx, ty, tw, th, B)
...
function Font.drawBox(tx, ty, tw, th, fill)
  ...
  drawBorder(tx, ty, tw, th, Font.BORDER)
end

-- SpriteBatch cache for the box border glyphs.  A battle redraws its message
-- box every frame; the border tiles are identical frame to frame (until
-- Font.setFrame swaps the style), so baking them into one batch turns ~48
-- separate glyph draws -- on the 3DS each one a Lua->C++ round trip -- into
-- a single love.graphics.draw(SpriteBatch).  Keyed by box spec + frame style.
local boxBatchCache = {}

local function borderBatch(tx, ty, tw, th, B)
  -- every border glyph must live on the same page to fit one SpriteBatch
  local list, page = {}, nil
  local function add(code, x, y)
    local p = pageFor(code)
    if not p then return false end
    if page and p ~= page then return false end
    page = p
    list[#list + 1] = { code = code, x = x, y = y }
    return true
  end
  if not add(B.tl, tx * 8, ty * 8) then return nil end
  if not add(B.tr, (tx + tw - 1) * 8, ty * 8) then return nil end
  if not add(B.bl, tx * 8, (ty + th - 1) * 8) then return nil end
  if not add(B.br, (tx + tw - 1) * 8, (ty + th - 1) * 8) then return nil end
  for i = 1, tw - 2 do
    if not add(B.h, (tx + i) * 8, ty * 8) then return nil end
    if not add(B.h, (tx + i) * 8, (ty + th - 1) * 8) then return nil end
  end
  for j = 1, th - 2 do
    if not add(B.v, tx * 8, (ty + j) * 8) then return nil end
    if not add(B.v, (tx + tw - 1) * 8, (ty + j) * 8) then return nil end
  end
  if not page or not page.quads then return nil end
  local batch = love.graphics.newSpriteBatch(page.image, #list, "dynamic")
  for _, e in ipairs(list) do
    batch:add(page.quads[e.code - page.base], e.x, e.y)
  end
  return batch
end

-- drawBox variant with the border cached in a SpriteBatch.  Identical
-- visuals; falls back to per-glyph drawing when the border page can't be
-- batched (e.g. a page spanning several sheets).
function Font.drawBoxBatched(tx, ty, tw, th, fill)
  local r, g, b, a = love.graphics.getColor()
  if type(fill) == "table" and fill[1] and fill[2] and fill[3] then
    love.graphics.setColor(fill[1] / 255, fill[2] / 255, fill[3] / 255, 1)
  else
  ...
  love.graphics.setColor(r, g, b, a)
  local B = Font.BORDER
  local key = tx .. "," .. ty .. "," .. tw .. "," .. th .. "," .. currentFrame
  local batch = boxBatchCache[key]
  if batch == nil then
    batch = borderBatch(tx, ty, tw, th, B)
    boxBatchCache[key] = batch or false
  end
  if batch then
