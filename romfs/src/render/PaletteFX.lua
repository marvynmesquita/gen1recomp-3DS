...
function PaletteFX.setMode(mode)
  ...
  if prev ~= PaletteFX.mode then
    ...
    -- RED++'s baked tileset atlas is built once per map, so a mode or palette
    -- change only needs the live map and its connected neighbors invalidated.
    -- A full invalidateAll rebuilds the entire resident set on a colors toggle,
    -- which is exactly the kind of transition spike visible indoors / outdoors.
    pcall(function()
      local Game = require("src.core.Game")
      local MapLoader = require("src.world.MapLoader")
      local ow = Game.overworld
      if ow and ow.map then
        MapLoader.invalidate(ow.map.id)
        for _, nb in ipairs(ow.neighbors or {}) do
          MapLoader.invalidate(nb.map.id)
        end
        if ow.reloadMap then ow:reloadMap(ow.map.id, "colors") end
      end
    end)
  end
end
