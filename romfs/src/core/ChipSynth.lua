...
-- one 512-sample stereo SoundData is the unit both the worker hands off and
-- the synchronous fallback queues; the source keeps MUSIC_BUFFER_COUNT of them
-- (~3s at 22050) for stall tolerance (window resize, a long GC pause, and the
-- 3DS synth worker's thin core-1 CPU slice -- a deeper queue smooths over the
-- periodic underruns that showed up as music stutter)
local MUSIC_BUFFER_SAMPLES = 512
local MUSIC_BUFFER_COUNT = 128
...
local function snapTicks(ticks)
  return math.floor((ticks * 1470 + 512) / 1024)
end
...
function Channel:resetNoise()
  ...
  self.noiseClock = 0
  if self.engine.synth then
    self.engine.synth:resetNoise(self.index)
  end
end
...
-- With the C core, Lua advances each channel's event only when the C renderer
-- hits a boundary (eventSample >= event.samples).  Mirrors the advance rules
-- of Channel:sample() exactly, including drum tails.
function Engine:advanceBoundaries()
  if self.synth then
    self.synth:setChannelVolumes({
      ChipSynth.getChannelVolume(1), ChipSynth.getChannelVolume(2),
      ChipSynth.getChannelVolume(3), ChipSynth.getChannelVolume(4),
    })
    self.synth:setChannelPitches({
      ChipSynth.getChannelPitch(1), ChipSynth.getChannelPitch(2),
      ChipSynth.getChannelPitch(3), ChipSynth.getChannelPitch(4),
    })
  end
  for _, channel in ipairs(self.channels) do
    while not channel.ended
        and (not channel.event
          or self.synth:eventSample(channel.index) >= channel.event.samples) do
      local prev = channel.event
      if prev then prev.sample = prev.samples end
      channel.event = channel:nextEvent()
      channel.phase = 0
      if channel.event and channel.event.drum then
        channel.drumTail = nil
        channel:resetNoise()
      elseif prev and prev.drum and prev.sample < drumAudioEnd(prev.drum) then
        channel.drumTail = prev
      elseif not (channel.event and channel.event.silence and channel.drumTail) then
        channel.drumTail = nil
        channel:resetNoise()
      end
      local waveTable
      if channel.event and channel.event.wave then
        waveTable = self.waves[
          math.min(channel.event.waveInstrument + 1, #self.waves)]
      end
      self.synth:setEvent(channel.index, channel.event, channel.drumTail,
        channel.drumTail and channel.drumTail.sample or 0, waveTable)
    end
  end
end

-- Streaming with the C core: render exactly `samples` frames, advancing
...
local function soundDataC(engine, samples, channels)
  ...
  while rendered < samples do
    ...
    local count = engine.synth:render(
      result, rendered, samples - rendered, channels == 2)
...
function Engine.new(data, header, options)
  ...
  -- C synthesis core (love.synth): when available, per-sample rendering runs
  -- in C and Lua only advances events at channel boundaries.
  if love.synth then
    local hwList = {}
    for _, spec in ipairs(channels) do
      hwList[#hwList + 1] = (spec.number - 1) % 4 + 1
    end
    engine.synth = love.synth.new(SAMPLE_RATE, hwList)
  end
  ...
  for _, spec in ipairs(channels) do
    ...
    engine.channels[#engine.channels].index = #engine.channels
  end
  return engine
end
...
local function soundData(engine, samples, channels)
  if engine.synth then return soundDataC(engine, samples, channels) end
  local result = love.sound.newSoundData(samples, SAMPLE_RATE, 16, channels)
...
local function renderEffectData(data, header, options)
  ...
  local count = 0
  if engine.synth then
    local scratch = love.sound.newSoundData(maximum, SAMPLE_RATE, 16, 2)
    while count < maximum and not engine:finished() do
      engine:advanceBoundaries()
      local chunk = engine.synth:render(scratch, count, maximum - count, false)
    ...
    for index = 0, count - 1 do
      local value = scratch:getSample(index, 1)
      result:setSample(index, 1, value)
      result:setSample(index, 2, value)
    end
    return result
  end
  local values = {}
