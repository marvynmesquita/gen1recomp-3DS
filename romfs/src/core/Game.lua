...
-- so a missing module can NEVER break the game
...
function Game:load()
  ...
  self._lastOptionsWriteAt = 0
  self._pendingOptionsWrite = false
  self._lastSaveWriteAt = 0
  self._pendingSaveWrite = false
...
function Game:step(dt)
  ...
  -- Spread expensive asset creation across frames instead of doing it all in one
  -- transition burst. This is the safe, incremental next step toward a workered
  -- pipeline: it lowers frame spikes without creating a shared-game-state race.
  if require("src.render.Assets").update then
    require("src.render.Assets").update()
  end
  -- Flush pending writes after a short grace period. This keeps the 3DS SD card
  -- from being hammered by save/option writes created by repeated transitions or
  -- hotkeys while still ensuring the final state lands eventually.
  if self._pendingSaveWrite or self._pendingOptionsWrite then
    local now = love and love.timer and love.timer.getTime and love.timer.getTime() or 0
    if self._pendingSaveWrite and (now - self._lastSaveWriteAt) >= 1.0 then
      self:writeSave(true)
    end
    if self._pendingOptionsWrite and (now - self._lastOptionsWriteAt) >= 0.75 then
      self:writeOptions(true)
    end
  end
...
function Game:update(dt)
  if DrawProf then DrawProf.begin("upd_total") end
...
function Game:draw()
  if DrawProf then DrawProf.begin("gd_pre") end
  ...
  if DrawProf then DrawProf.finish("gd_pre") end
  ...
  if DrawProf then DrawProf.begin("gd_zones") end
  ...
  -- 3DS has no shader support: zone lists exist only to drive the shade-
  -- remap shader in blitCanvas, so skip the whole scan there (nil zones
  -- fall through to its unshaded fast path).  Desktop/mobile keep it.
  local zones, worldZones, zoneOwner
  local canShade = require("src.render.PaletteFX").shader() ~= nil
  if canShade then
  ...
  if canShade and worldDrawn and self.overworld.sgbWorldZones then
    worldZones = self.overworld:sgbWorldZones()
  end
  if DrawProf then DrawProf.finish("gd_zones") end
  if DrawProf then DrawProf.begin("ef_end") end
  local viewport = Renderer:endFrame(zones, worldZones)
  if DrawProf then DrawProf.finish("ef_end") end
  if DrawProf then DrawProf.begin("gd_hud") end
  ...
  TouchControls:draw()
  if DrawProf then DrawProf.finish("gd_hud") end
end
...
function Game:writeSave(force)
  if not self.save then return false end
  local now = love and love.timer and love.timer.getTime and love.timer.getTime() or 0
  if not force and self._lastSaveWriteAt > 0 and (now - self._lastSaveWriteAt) < 1.5 then
    self._pendingSaveWrite = true
    return true
  end
  ...
  self._lastSaveWriteAt = now
  self._pendingSaveWrite = false
  return SaveData.save(self.save)
end

-- Persist options.lua only (Options menu / hotkeys 2-5).  Keeps settings
-- across New Game without touching the progress save.  This port is on SD
-- storage, so repeated menu/hotkey writes would otherwise hammer the card;
-- coalesce them and let a forced flush happen only for shutdown or explicit
-- critical writes.
function Game:writeOptions(force)
  if not (self.save and self.save.options) then return end
  local now = love and love.timer and love.timer.getTime and love.timer.getTime() or 0
  if not force and self._lastOptionsWriteAt > 0 and (now - self._lastOptionsWriteAt) < 0.5 then
    self._pendingOptionsWrite = true
    return
  end
  self._lastOptionsWriteAt = now
  self._pendingOptionsWrite = false
