...
function YellowIntro.new(game, onDone)
  ...
  self.pendingDelay = 0
  self.waveQuads = nil    -- pre-allocated scanline quads (scene 7 wave)
  self.waveQuadsW = nil   -- wrap-around half (only filled when sx > 0)
...
function YellowIntro:startScene(scene)
  ...
  elseif scene == 6 then
    ...
    -- pre-allocate 144 scanline quads so the draw loop does not create
    -- new GPU objects every frame (avoids OOM / data-abort on 3DS)
    t.waveQuads = {}
    t.waveQuadsW = {}
    for i = 0, 143 do
      t.waveQuads[i]  = love.graphics.newQuad(0, 0, 160, 1, 256, 256)
      t.waveQuadsW[i] = love.graphics.newQuad(0, 0, 160, 1, 256, 256)
    end
    t:bgFill(0x10)
    for y = 0, 2 do
...
function YellowIntro:draw()
  ...
  if self.bgCanvas then
    if self.wave then
      ...
      -- Reuse pre-allocated quads via setViewport instead of creating
      -- new GPU objects every frame (3DS memory-safe, #523).
      local cw, ch = self.bgCanvas:getDimensions()
      local sx = self.scx % 256
      local w1 = math.min(160, 256 - sx)
      local q = self.waveQuads
      local qw = self.waveQuadsW
      for ly = 0, 143 do
        local dy = (ly >= 16 and ly < 128) and self.wave[ly] or 0
        local sy = (ly + dy) % 256
        q[ly]:setViewport(sx, sy, w1, 1, cw, ch)
        love.graphics.draw(self.bgCanvas, q[ly], 0, ly)
        if w1 < 160 then
          qw[ly]:setViewport(0, sy, 160 - w1, 1, cw, ch)
          love.graphics.draw(self.bgCanvas, qw[ly], w1, ly)
        end
      end
    else
      local cw, ch = self.bgCanvas:getDimensions()
