...
-- title sits on white paper, so its static art recedes while the
-- centerpiece mon and Red pop toward the viewer -- relative depth between
-- the planes is what reads as 3D, not a uniform shift (which just looks
-- like the screen shrank).  _parallax is reset to 0 at the end so nothing
-- drawn after the title inherits a stale depth.
local P_BG    = -0.6  -- white paper
local P_LOGO  = -0.4  -- logo / brand / version ribbon / yellow bubble
local P_HERO  =  0.2  -- big mon (or Yellow's Pikachu + blink)
local P_FRONT =  0.6  -- Red + ball, copyright
...
function TitleState:draw()
  ...
  if self.yellowLayout then
    ...
    love.graphics._parallax = P_HERO
    ...
    love.graphics._parallax = P_FRONT
