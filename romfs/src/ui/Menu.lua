...
-- Stereoscopic 3D: a NON-OPAQUE menu (the START menu) draws over the live
-- overworld, whose world pass already has per-layer parallax.  Give the menu
-- its own gentle parallax (game px at full 3D slider) so it floats in 3D
-- too, instead of snapping flat to the screen plane while the map behind it
-- has depth.  Opaque full-screen menus (party, bag, battle) replace the world
-- entirely, so they keep drawing flat.  The C++ draw adds
-- slider * _parallax * eyeSign to every draw's X, exactly like the world.
local PARALLAX_MENU = 0.5
...
function Menu:draw()
  ...
  -- Stereoscopic 3D: this menu floats over the live overworld only when a
  -- world pass drew beneath it this frame (renderer.worldActive -- set by
  -- Renderer:beginWorldPass and false for opaque full-screen states like
  -- the party menu / bag / battle, which replace the world and stay flat).
  -- Publish a gentle parallax so the menu box/text get depth too.
  local menuParallax = 0
  if self.game and self.game.renderer and self.game.renderer.worldActive
     and love.graphics and love.graphics._3dSlider
     and love.graphics._3dSlider > 0 then
    menuParallax = PARALLAX_MENU
    love.graphics._parallax = menuParallax
  end
  ...
  -- reset the per-draw parallax so whatever draws after this menu (a text
  -- box, another overlay) does not inherit the menu's depth layer
  if menuParallax ~= 0 then love.graphics._parallax = 0 end
end
